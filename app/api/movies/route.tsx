...
export async function GET(request: Request) {
  ...
  let url = "";
  if (popular === "true") {
    url = `https://api.themoviedb.org/3/movie/popular?language=${lang}&page=${page}`;
  } else if (query) {
    url = `https://api.themoviedb.org/3/search/movie?query=${encodeURIComponent(query)}&language=${lang}&page=${page}`;
  } else {
